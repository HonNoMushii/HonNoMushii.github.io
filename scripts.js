    ...
    const fetchAPOD = () => {
        fetch('data/apod.json')
            ...
            .then(data => {
                ...
                const apodData = data[randomIndex];
                let apodContent = `<h3>${apodData.title}</h3>`;
                if (apodData.media_type === 'video') {
                    apodContent += `<iframe src="${apodData.url}" frameborder="0" allowfullscreen style="max-width: 100%; height: 400px;"></iframe>`;
                } else {
                    apodContent += `<img src="${apodData.url}" alt="${apodData.title}" style="max-width: 100%;">`;
                }
                apodContent += `<p>${apodData.explanation}</p>`;
                apodElement.innerHTML = apodContent;
            });
    };
